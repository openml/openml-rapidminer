	...
	private int getExampleFlowId() {
		...
		if(example_flow.exists() && example_flow.isFile()) {
			try {
				...
				// Check the reason why the flow failed to upload
				if(message.contains("flow already exists")) {
					// If the flow is already added in OpenML, get the id
